...
export default function ListPage() {
  ...
  const badgeholderAttestationUid = address
    ? getBadgeholderAttestationUid(address)
    : undefined;
  ...
  const fetchList = useCallback(async () => {
    ...
    const response = await axios.get(
      "/dataset/rpgf3/lists/" + listId + ".json"
    );
  ...
  const badgeholderApproveFn = useCallback(async () => {
    try {
      ...
      const rawSignature = await listAttestSignature(
        list._id,
        list.listName,
        signer,
        list.attestationUid
    ...
    <Layout>
      ...
      <div>
        <div className="mb-4">
          ...
          {!isAgora && (
            <>
              <div className="text-[#858796] text-sm mb-3">
                By {list.domainName}
              </div>
              ...
              <div className="flex gap-2 items-center">
                <Tooltip title={list.twitter}>
                  <a
                    href={"https://twitter.com/" + list.twitter}
                    target="_blank"
                    className="transition hover:cursor-pointer hover:scale-110"
                  >
                    <img
              ...
              <div className="mb-4">
                <Alert
                  message={
                    address && isConnected
                      ? "For Badgeholders"
                      : "Are you a badgeholder?"
                  }
                  description={
                    address && isConnected ? (
                      <div>
                        <div className="mb-2">
                          Please approve this list if you want to include it in
                          your ballot
                        </div>
                        <div>
                          <PrimaryButton
                      ...
                      <div>
                        <div className="mb-2">
                          If you are a badgeholder, connect your wallet to
                          approve this list
                        </div>
                        <div>
        ...
        {!isAgora && RETROLIST_SECRET && (
          <div className="flex gap-3 mb-4">
            <PrimaryButton onClick={() => qualifyAction("qualify")}>
              Qualify
            </PrimaryButton>
            <SecondaryButton onClick={() => qualifyAction("unqualify")}>
              Reset
            </SecondaryButton>
            <PrimaryButton onClick={() => qualifyAction("ban")}>
              BAN
            </PrimaryButton>
          </div>
        )}
