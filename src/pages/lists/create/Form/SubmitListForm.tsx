...
import { listContentView, listPieChart, rubricTotalScore } from "../../../../utils/list";
        ...
        items={listContent.map(project => (
          {
                    ...
                    <div className="flex gap-3 w-1/3">
                      <div>
                        <img
                          width={40}
                          height={40}
                          src={project.project?.profileImageUrl || "/img/project-placeholder.svg"}
                          className="rounded-full"
                          alt=""
                        />
                      </div>
                      <div>
                        <div className="font-bold">{project.project?.displayName}</div>

                        <a href={`https://retrolist.app/project/${project.RPGF3_Application_UID}`} target="_blank">
            ...
            children: (
              <div className="ml-4">
                {state.rubric?.criteria.map((criteria) => (
                  project.evaluation[criteria._id] && (
                    <ItemSubmitListForm
                      title={criteria.title}
                      score={project.evaluation[criteria._id].score}
                      description={criteria.scores[project.evaluation[criteria._id].score] || 'ERROR'}
